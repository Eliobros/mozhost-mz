...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  passport.use(new GoogleStrategy({
    ...
    callbackURL: process.env.GOOGLE_CALLBACK_URL || 'https://api.mozhost.shop/api/auth/google/callback'
    ...
    try {
      ...
      const result = await database.query(
        `INSERT INTO users (username, email, password_hash, oauth_provider, oauth_provider_id, avatar_url, email_verified, profile_completed, plan, max_containers, max_ram_mb, max_storage_mb, coins, free_trial_ends)
         VALUES (?, ?, NULL, 'google', ?, ?, true, false, 'free', 1, 512, 1024, 0, DATE_ADD(NOW(), INTERVAL 7 DAY))`,
...
if (process.env.GITHUB_CLIENT_ID && process.env.GITHUB_CLIENT_SECRET) {
  passport.use(new GitHubStrategy({
    ...
    callbackURL: process.env.GITHUB_CALLBACK_URL || 'https://api.mozhost.shop/api/auth/github/callback',
    scope: ['user:email']
  }, async (accessToken, refreshToken, profile, done) => {
    try {
      const email = profile.emails && profile.emails[0] ? profile.emails[0].value : null;
      ...
      const result = await database.query(
        `INSERT INTO users (username, email, password_hash, oauth_provider, oauth_provider_id, avatar_url, email_verified, profile_completed, plan, max_containers, max_ram_mb, max_storage_mb, coins, free_trial_ends)
         VALUES (?, ?, NULL, 'github', ?, ?, true, false, 'free', 1, 512, 1024, 0, DATE_ADD(NOW(), INTERVAL 7 DAY))`,
