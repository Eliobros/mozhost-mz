...
// ==================== COMPONENTE INTERNO ====================
const WebTerminalContent = () => {
  // Hook do Toast - DEVE VIR PRIMEIRO!
  ...
  const connectToTerminal = () => {
    ...
    const newSocket = io('https://api.mozhost.shop', {
      auth: {
        token: token
      }
    });
    ...
    newSocket.on('connect', () => {
      ...
      setConnected(true);
      toast.success('✅ Conectado ao terminal!');
    ...
    newSocket.on('terminal-exit', (data) => {
      ...
      setConnected(false);
      toast.warning('⚠️ Terminal encerrado');
    });

    newSocket.on('disconnect', () => {
      console.log('Disconnected from terminal server');
      setConnected(false);
      toast.warning('⚠️ Desconectado do terminal');
    });
...
`;
  ...
  const sendCommand = (command) => {
    if (!socket || !connected) {
      ...
      toast.error('❌ Terminal não conectado');
      return;
    }
  ...
  const changeContainer = (container) => {
    ...
    setCommandHistory([]);
    setHistoryIndex(-1);
    toast.info(`🔄 Mudando para container: ${container.name}`);
  };

  const reconnectTerminal = () => {
    toast.info('🔄 Reconectando ao terminal...');
    if (socket) {
      socket.disconnect();
      setSocket(null);
    }
  ...
  const copyOutput = () => {
    ...
    toast.success('✅ Output copiado para área de transferência!');
  };
