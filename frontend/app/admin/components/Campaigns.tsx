...
export const Campaigns: React.FC<CampaignsProps> = ({ password }) => {
  ...
  const [tipo, setTipo] = useState('abandonados');

  const grupos = [
    { id: 'abandonados', label: '😴 Inativos / Abandonados' },
    { id: 'inativos', label: '🚫 Contas desativadas' },
  ...
  const campaignUrl = (path: string) =>
    `${API_ROOT}/api/admin/campaigns/${path}?password=${encodeURIComponent(password)}`;

  // Carrega template salvo
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        const res = await fetch(campaignUrl('template'), { headers: campaignHeaders });
  ...
  const salvarTemplate = async () => {
    ...
    setErro('');
    try {
      const res = await fetch(campaignUrl('template'), {
        method: 'POST',
        headers: campaignHeaders,
        body: JSON.stringify({ assunto, corpo, password })
      });
      const data = await res.json();
  ...
  const enviarCampanha = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(campaignUrl('send'), {
        method: 'POST',
        headers: campaignHeaders,
        body: JSON.stringify({ password, tipo, assunto, corpo })
      });
      const data = await res.json();
      if (!data.success) throw new Error(data.message);
      setResultado(data);
    } catch (err: any) {
      setErro(err.message || 'Erro ao enviar campanha');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Envia emails para usuários inativos, abandonados ou outros grupos</p>
      </div>
